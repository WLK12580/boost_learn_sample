...
int main() {
	...
	auto RecvData_Bind_Fun=std::bind(&threadmanager::RecvData,shared_ptr,std::placeholders::_1);
	std::future<std::string> func=std::async(std::launch::async,RecvData_Bind_Fun,"bind_data");
	...
	std::cout<<"my_data:"<<my_data<<" bind_data:"<<data<<std::endl;
